...
const aggregateDataByDay = (data) => {
  ...
  data.forEach(entry => {
    ...
    formattedData[key] = (formattedData[key] || 0) + 1; // Assuming count per entry
  });
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json({
      rendezvousByDay: aggregateDataByDay(rendezvous),
      consultationByDay: aggregateDataByDay(consultations),
    });
  } catch (error) {
